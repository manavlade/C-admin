...
export function PatientForm() {
  ...
  const [primaryConsultantId, setPrimaryConsultantId] = useState<string>("")
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log({
      ...
      address,
      medicalHistory,
      primaryConsultantId: primaryConsultantId === "" ? null : primaryConsultantId, // Send null if no consultant is selected
    })
    toast({
      title: "Patient Added!",
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 items-center gap-4">
        <Label htmlFor="primaryConsultant" className="md:text-right">
          Primary Consultant (Optional)
        </Label>
        <Select
          onValueChange={(selectedValue) => {
            if (selectedValue === "unassigned-consultant") {
              setPrimaryConsultantId("")
            } else {
              setPrimaryConsultantId(selectedValue)
            }
          }}
          value={primaryConsultantId === "" ? "unassigned-consultant" : primaryConsultantId}
        >
          <SelectTrigger className="md:col-span-3">
            <SelectValue placeholder="Assign primary consultant" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="unassigned-consultant">None</SelectItem>
